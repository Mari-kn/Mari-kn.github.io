...
class StarField {
    constructor() {
        ...
        // Shooting stars array for menu hover effects
        this.shootingStars = [];
        this.maxShootingStars = 3; // Limit concurrent shooting stars for performance
    ...
    setupEventListeners() {
        ...
        // Setup navigation hover effects for shooting stars
    ...
    setupNavigationHoverEffects() {
        ...
        navLinks.forEach(link => {
            ...
            link.addEventListener('touchstart', (e) => {
                // Prevent multiple stars on touch
    ...
    createShootingStar() {
        // Limit number of concurrent shooting stars
        if (this.shootingStars.length >= this.maxShootingStars) {
            return;
        }
        
        // Random starting position at top of screen
        const startX = Math.random() * this.canvas.width;
        const startY = -50; // Start above visible area
        
        // Random angle between 15 and 75 degrees
        const angle = (Math.random() * 60 + 15) * Math.PI / 180;
        
        // Random speed and length
        const speed = Math.random() * 15 + 10; // 10-25 pixels per frame
        const length = Math.random() * 100 + 50; // 50-150 pixel trail
        ...
        const shootingStar = {
            ...
            color: Math.random() > 0.5 ? '#64ffda' : '#ffffff', // Random color
            size: Math.random() * 2 + 1 // 1-3 pixel size
        };
    ...
    updateShootingStars() {
        ...
        this.shootingStars = this.shootingStars.filter(star => {
            ...
            // Limit trail length
            if (star.trail.length > star.length / star.speed) {
                star.trail.shift();
            }
    ...
    drawShootingStars() {
        this.shootingStars.forEach(star => {
            ...
            if (star.trail.length > 1) {
                ...
                const trailStart = star.trail[0];
                const trailEnd = star.trail[star.trail.length - 1];
                ...
                // Draw trail as connected lines with varying opacity
                ...
                headGlow.addColorStop(0, `${star.color}FF`);
                headGlow.addColorStop(0.5, `${star.color}44`);
