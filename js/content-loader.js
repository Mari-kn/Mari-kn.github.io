...
class ContentLoader {
    ...
    initializeExperienceAnimations() {
        ...
        // Sophisticated intersection observer with iOS-optimized settings
        const experienceObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    this.animateExperienceItem(entry.target);
                    // Don't unobserve on iOS to handle re-triggering
                    if (!isIOS) {
            ...
            threshold: isIOS ? 0.1 : 0.2, // Lower threshold for iOS
            rootMargin: isIOS ? '50px 0px -50px 0px' : '0px 0px -80px 0px' // Less aggressive margins for iOS
        });
    ...
    setupContentObservers() {
        ...
        // Content observer for newly loaded elements with iOS optimization
        const contentObserver = new IntersectionObserver((entries) => {
            ...
            threshold: isIOS ? 0.1 : 0.2, // Lower threshold for iOS
            rootMargin: isIOS ? '25px 0px -25px 0px' : '0px 0px -50px 0px' // Smaller margins for iOS
        });
